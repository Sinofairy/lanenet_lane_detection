...
class ASPP(nn.Module):
    ...
    def forward(self, feature_map):
        ...
        # (shape: (batch_size, 256, h/16, w/16)) # 此时的输入和输出的宽和高是一样的
        ...
        # (shape: (batch_size, 256, h/16, w/16))
        out_img = F.interpolate(out_img, size=(
            feature_map_h, feature_map_w), align_corners=False)

        # (shape: (batch_size, 1280, h/16, w/16)) # 1280 = 256*4+AdaptiveAvgPool2d的256
...
class ASPP_Bottleneck(nn.Module):
    ...
    def forward(self, feature_map):
        ...
        # (shape: (batch_size, 256, h/16, w/16))
        out_img = F.interpolate(out_img, size=(
            feature_map_h, feature_map_w), mode="bilinear") # mode="bilinear" 使用双线性插值恢复分辨率
